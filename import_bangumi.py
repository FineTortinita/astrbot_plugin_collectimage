...
def main():
    if len(sys.argv) < 2:
        print(__doc__)
        print("\n支持的文件格式:")
        print("  - .jsonlines (JSON Lines)")
        print("  - .jsonl (JSON Lines)")
        print("  - .json (JSON Array)")
        print("  - .zip (包含上述文件)")
        print("\n需要以下文件:")
        print("  - subject.jsonlines (作品数据)")
        print("  - subject-characters.jsonlines (作品角色关联)")
        print("  - character.jsonlines (角色数据)")
        sys.exit(1)
    
    dump_dir = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else "aliases.json"
    
    print(f"正在扫描: {dump_dir}")
    ...
    if "zip" in files:
        if not valid_character_ids:
            ...
            if subject_chars:
                filtered_subject_chars = [sc for sc in subject_chars if sc.get("subject_id") in valid_subject_ids]
                valid_character_ids = {sc.get("character_id") for sc in filtered_subject_chars if sc.get("character_id")}
                print(f"有效 character_id: {len(valid_character_ids)}")
    ...
    print(f"完成! 共 {total} 个别名 (角色: {len(result['character'])}, 作品: {len(result['work'])})")
